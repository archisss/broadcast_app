...
export const DoctorsManagementPage: React.FC = () => {
  ...
  const [passwordModalDoctor, setPasswordModalDoctor] = useState<Doctor | null>(null);
  ...
  const handleOpenPasswordModal = (doc: Doctor) => {
    if (!isSuperadmin && doc.role === 'superadmin') {
      setError('Un Administrador General no tiene permisos para modificar la contraseña de un Superadministrador.');
      setTimeout(() => setError(null), 4000);
      return;
    }
    setPasswordModalDoctor(doc);
    setNewPasswordValue(doc.password || '');
  ...
  const handleGenerateRandomPassword = () => {
    ...
    setNewPasswordValue(res);
  };

  const handleSaveNewPassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!passwordModalDoctor) return;
    ...
    try {
      const res = await fetch(`/api/doctors/${passwordModalDoctor.id}`, {
        method: 'PATCH',
        headers: {
      ...
      setDoctors((prev) =>
        prev.map((d) => (d.id === passwordModalDoctor.id ? { ...d, password: trimmed } : d))
      );
      ...
      setPasswordModalError(err.message || 'Error al guardar nueva contraseña');
    } finally {
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    {passwordModalDoctor.name} (@{passwordModalDoctor.username})
                  </p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleSaveNewPassword} className="space-y-4">
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button
                    ...
                    className="text-[11px] font-medium text-sky-600 hover:text-sky-800 hover:underline cursor-pointer"
                  >
                    Generar aleatoria
                  </button>
                </div>
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-xs text-slate-900 pr-10 focus:outline-none focus:ring-2 focus:ring-sky-500 font-mono"
                    autoFocus
                  />
                  <button
                    type="button"
                ...
                <p className="text-[11px] text-slate-400 mt-1">
                  El usuario podrá iniciar sesión inmediatamente con esta nueva clave.
                </p>
              </div>
              ...
              <div className="pt-3 flex items-center justify-end gap-3 border-t border-slate-100">
                ...
                <button
                  ...
                  className="px-5 py-2.5 rounded-xl bg-sky-700 hover:bg-sky-600 text-white text-xs font-bold shadow-md transition-all disabled:opacity-50 cursor-pointer"
                >
